...
export default async function({faker}, target, that, args) {
  ...
  //Commit content
  if (/^https:..api.github.com.repos.Shadowghost.gh-metrics.commits.MOCKED_SHA/.test(url)) {
    console.debug(`metrics/compute/mocks > mocking rest api result > rest.request ${url}`)
    return ({
      status: 200,
      url: "https://api.github.com/repos/Shadowghost/gh-metrics/commits/MOCKED_SHA",
      data: {
        sha: "MOCKED_SHA",
        commit: {
          author: {
